...
import { BookOpen, Pen, Mic, Headphones, Check, X, RotateCcw, ArrowRight, Lightbulb, Play, Pause, MessageSquare, Map as MapIcon, LayoutGrid, MessagesSquare } from "lucide-react";
...
import { WORDS, getWords, getQuizzes, getSpeaking, getListening, getRealConversations } from "../data/content";
...
import type { Language, WordFamily, ReviewQuality, DialogueScene, RealConversation } from "../types";

type Tab = "words" | "grammar" | "speaking" | "listening" | "review" | "conversation" | "real-conversation";
...
export default function LearnPage() {
  ...
  const TABS: { key: Tab; label: string; icon: React.ElementType; desc: string }[] = [
    ...
    { key: "real-conversation", label: t("learn.realConversationLabel"), icon: MessagesSquare, desc: t("learn.realConversationDesc") },
  ];
    ...
    >
      ...
      {view === "modules" && (
        <>
          ...
          {tab === "real-conversation" && <RealConversationModule language={lang} isLoggedIn={isLoggedIn} onNeedLogin={onNeedLogin} />}
        </>
      )}
...
function RealConversationModule({ language, isLoggedIn, onNeedLogin }: { language: Language; isLoggedIn: boolean; onNeedLogin: () => void }) {
  const { t } = useTranslation();
  const [domain, setDomain] = useState<string>("restaurant");
  const conversations = useMemo(() => getRealConversations(language, domain), [language, domain]);
  ...
  const conv: RealConversation | null = conversations[convIdx % Math.max(1, conversations.length)] ?? null;
  ...
  const handleScored = (score: number) => {
    ...
    setBestScores((prev) => ({ ...prev, [uttIdx]: Math.max(prev[uttIdx] ?? 0, score) }));
  };
  ...
  const totalAssistantUtts = utterances.filter((u) => u.speaker === "ASSISTANT").length;
  const progress = totalAssistantUtts > 0 ? Math.round((completedInConv / totalAssistantUtts) * 100) : 0;
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <GlassCard className="flex min-h-[380px] flex-col items-center justify-center py-12">
            ...
            <div className="mt-4 text-sm text-brand-200/70">{t("learn.realConv.empty")}</div>
            <div className="mt-1 text-xs text-brand-200/50">{t("learn.realConv.emptyHint")}</div>
          </GlassCard>
        </div>
        <div className="space-y-4">
          <GlassCard className="p-5">
            <div className="text-xs uppercase tracking-wider text-brand-200/60">{t("learn.realConv.aboutTitle")}</div>
            <div className="mt-2 text-xs text-brand-100/80">{t("learn.realConv.about")}</div>
          </GlassCard>
        </div>
      </div>
    );
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      <div className="lg:col-span-2">
        <GlassCard>
          ...
          <div className="mb-4 flex flex-wrap gap-2">
            {REAL_CONV_DOMAINS.map((d) => (
              <button
                ...
                className={
                  "rounded-full px-3 py-1.5 text-xs transition " +
                  (d.id === domain ? "bg-gradient-to-r from-sky-400 to-fuchsia-500 text-white" : "bg-white/5 text-brand-200/70 hover:text-white")
                }
              >
                {t(d.labelKey)}
              </button>
          ...
          <div className="mb-4 flex items-center justify-between text-xs text-brand-200/70">
            <button onClick={prev} className="rounded-md bg-white/5 px-2 py-1 hover:bg-white/10">← {t("learn.realConv.prevConv")}</button>
            <span>{t("learn.realConv.conversationN", { n: convIdx + 1, total: conversations.length })}</span>
            <button onClick={() => { setConvIdx((n) => n + 1); setUttIdx(0); setBestScores({}); }} className="rounded-md bg-white/5 px-2 py-1 hover:bg-white/10">{t("learn.realConv.nextConv")} →</button>
          </div>
          ...
          {currentUtt && (
            <div className="rounded-2xl border border-white/10 bg-black/20 p-6">
              <div className="flex items-center justify-between text-[11px] uppercase tracking-wider text-brand-200/60">
                <span>{currentUtt.speaker === "ASSISTANT" ? t("learn.realConv.assistantTurn") : t("learn.realConv.userTurn")}</span>
                <span>{uttIdx + 1} / {utterances.length}</span>
              </div>
              ...
              {currentUtt.speaker === "ASSISTANT" ? (
                <div className="mt-5">
                  {isLoggedIn ? (
                    ...
                    >
                      <Mic className="h-4 w-4" /> {t("learn.speakingModule.loginToStart")}
                    </button>
                  )}
                ...
                <div className="mt-4 text-xs text-brand-200/60">{t("learn.realConv.userTurnHint")}</div>
              )}
            </div>
          )}
      ...
      <div className="space-y-4">
        <GlassCard className="p-5">
          <div className="text-xs uppercase tracking-wider text-brand-200/60">{t("learn.realConv.sessionTitle")}</div>
          <div className="mt-2 font-display text-3xl font-bold text-white">{avgScore}</div>
          <div className="text-xs text-brand-200/60">{t("learn.realConv.avgScore")}</div>
